...
ok( !$EVAL_ERROR, 'simple Dep constructed' );
...
ok( !$EVAL_ERROR, 'versioned Dep constructed' );
...
ok( !$EVAL_ERROR, 'versioned Dep parsed' );
...
ok( !$EVAL_ERROR, '<= dependency parsed' );
...
sub sat ($$$) {
   ...
   ok( $dep->satisfies($test) == $expected,
       "$dep ".($expected ? 'satisfies' : "doesn't satisfy"). " $test" );
}
...
sub comp {
   ...
   $one = Debian::Dependency->new($one); $two = Debian::Dependency->new($two);

   is( $one <=> $two, $expected,
       $expected ? ( ($expected == -1) ? "$one is less than $two"
